...
func (n *Node) generateObjectData(objectHeader *pb.ObjectHeader, minFiles, maxFiles uint32) error {
	...
	objectDirectoryPath := fmt.Sprintf("%s/%s/%s", n.fs.MountDir, STUDIES_DIR, objectHeader.GetName())
	...
	n.fs.InsertObjectHeader(objectHeader.GetName(), objectHeader)
	...
	for _, s := range objectHeader.GetMetadata().GetSubjects() {
		...
		n.fs.EraseObjectsFromSubject(s)
		subjectObjectsDirPath := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, SUBJECTS_DIR, s, STUDIES_DIR, objectHeader.GetName())

		n.fs.EraseObjectsFromSubject(objectHeader.GetName())
		n.fs.InsertSubject(s, objectHeader.GetName())
...
func (n *Node) setExtentedAttributes(subject string, objectHeader *pb.ObjectHeader, numFiles int) error {
	...
	dirPath := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, SUBJECTS_DIR, subject, STUDIES_DIR, objectHeader.GetName())
	...
	for i := 1; i <= numFiles; i++ {
		...
		for k, v := range policy {
			...
			if err := n.fs.SetExtendedAttribute(filePath, k, attrs); err != nil {
				panic(err)
			}
		}
	}
...
func (n *Node) storeMetaData(objectHeader *pb.ObjectHeader) error {
	...
	jsonPath := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, objectName, METADATA_DIR, METADATA_FILE)
...
func (n *Node) createObjectFileTree(objectID string) error {
	dirPath := fmt.Sprintf("%s/%s/%s", n.fs.MountDir, STUDIES_DIR, objectID)
	if err := createDirectory(dirPath); err != nil {
		return err
	}

	dirPath = fmt.Sprintf("%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, objectID, DATA_USER_DIR)
	if err := createDirectory(dirPath); err != nil {
		return err
	}

	dirPath = fmt.Sprintf("%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, objectID, METADATA_DIR)
	if err := createDirectory(dirPath); err != nil {
		return err
	}

	dirPath = fmt.Sprintf("%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, objectID, PROTOCOL_DIR)
	if err := createDirectory(dirPath); err != nil {
		return err
	}

	dirPath = fmt.Sprintf("%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, objectID, SUBJECTS_DIR)
	if err := createDirectory(dirPath); err != nil {
		return err
	}
...
func (n *Node) createSubjectObjectsDirectory(subject, object string) error {
	...
	dirPath := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, SUBJECTS_DIR, subject, STUDIES_DIR, object)
...
func (n *Node) createSubjectStudyFiles(subject, objectName string, numFiles, fileSize int) error {
	...
	dirPath := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, SUBJECTS_DIR, subject, STUDIES_DIR, objectName)
...
func (n  *Node) addSubjectStudyFilesToStudy(subject, study string, numFiles int) error {
	dirPath := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, study, SUBJECTS_DIR, subject)

	// Need a clean directory to begin with
	pathToDelete := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, study, SUBJECTS_DIR, subject)
	...
	filePathDir := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, SUBJECTS_DIR, subject, STUDIES_DIR, study)
	targetPathDir := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, study, SUBJECTS_DIR, subject)
	for i := 1; i <= numFiles; i++ {
...
func (self *Ptfs) SetSubjectPolicy(subject, object, filename, modelText string) error {
	...
	modelFilePath := fmt.Sprintf("%s/%s/%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, object, SUBJECTS_DIR, subject, filename)
...
/*func (self *Ptfs) StudyMetaData(msg pb.NodeMessage) ([]byte, error) {
	...
	jsonPath := fmt.Sprintf("%s/%s/%s/%s/%s", n.fs.MountDir, STUDIES_DIR, msg.Study, METADATA_DIR, METADATA_FILE)
