...
var (
	...
	errNoLocalCryptoUnitConfig = errors.New("No crypto unit config present")
	errNoPath             = errors.New("Crypto unit path is not set")
	errNoHostnames        = errors.New("Hostnames is not set")
)

type LocalCryptoUnit struct {
...
type LocalCryptoUnitConfig struct {
	Identity  pkix.Name
	CaAddr    string
	Hostnames []string
}

func NewLocalCryptoUnit(path string, config *LocalCryptoUnitConfig) (*LocalCryptoUnit, error) {
	if path == "" {
...
func validateConfiguration(config *LocalCryptoUnitConfig) error {
	if config == nil {
		return errNoLocalCryptoUnitConfig
	}
...
func (cu *LocalCryptoUnit) Certificate() *x509.Certificate {
	return cu.self
}

func (cu *LocalCryptoUnit) CaCertificate() *x509.Certificate {
	return cu.ca
}

func (cu *LocalCryptoUnit) PrivateKey() *ecdsa.PrivateKey {
	return cu.priv
}

func (cu *LocalCryptoUnit) PublicKey() *ecdsa.PublicKey {
